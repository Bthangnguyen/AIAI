...
async def run_pipeline_testcase(test_id: str, prompt: str, expected_desc: str):
    """Executes a single testcase through the whole pipeline."""
    print(f"\n========================================================")
    print(f"[TESTCASE] RUNNING TESTCASE: {test_id}")
    ...
    if test_id == "T081":
        ...
        def solve_callback(pois, hotels, days, matrix, time_limit, **kwargs):
            return solver_service.solver.solve_trip(
                pois=pois, hotels=hotels, days=days, matrix=matrix, time_limit=time_limit, **kwargs
            )
        ...
        for alt in alternatives:
            ...
            alt_days = solver_service._validate_budget(
                alt_days, l4_pois, l4_hotels, day_plans, constraints, matrix, 30, "ortools"
            )
        ...
        req = TravelPlanRequest(
            ...
            solver_config=SolverConfig(time_limit=30)
        )
        itinerary = solver_service.plan(req)
...
async def main():
    await run_pipeline_testcase(
        "T001", 
        "Toi muon di Hue 1 ngay, ngan sach 500k, thich Dai Noi va an bun bo.",
        "num_days=1, budget, locked Dai Noi, food=bun bo"
    )
    
    await run_pipeline_testcase(
        "T021",
        "Toi an chay, di Hue 2 ngay, nho xep quan chay vao bua trua.",
        "vegetarian, lunch window"
    )
    
    await run_pipeline_testcase(
        "T081",
        "Cho toi 3 phuong an: tiet kiem, can bang, chill cho lich trinh 3 ngay o Hue.",
        "multi-plan alternatives"
    )
    
    print("\nALL PIPELINE INTEGRATION TESTS RUN AND COMPLETED SUCCESSFULLY!")
