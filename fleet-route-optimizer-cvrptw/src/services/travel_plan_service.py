...
from ..models.errors import ErrorCode, SolverException
...
class TravelPlanService:
    ...
    def plan(
        ...
        if not self._lock.acquire(timeout=60.0):
            raise SolverException(ErrorCode.NO_FEASIBLE_ROUTE, "Travel solver is busy. Request timed out in queue.")
    ...
    def _plan_impl(
        ...
        # 1. Pre-flight validate locked POIs
        day_plans = request.day_plans or self._generate_day_plans(request)
        total_max_pois = sum(d.max_pois for d in day_plans)
        locked_pois = [p for p in request.pois if p.is_locked]
        
        if len(locked_pois) > total_max_pois:
            raise SolverException(
                ErrorCode.TOO_MANY_LOCKED,
                f"Bạn đang ghim quá nhiều điểm ({len(locked_pois)}). Số điểm ghim tối đa được phép là {total_max_pois}."
            )

        total_locked_duration = sum(p.visit_duration_min for p in locked_pois)
        total_time_budget = sum(d.max_daily_minutes for d in day_plans)
        if total_locked_duration > total_time_budget:
            raise SolverException(
                ErrorCode.TOO_MANY_LOCKED,
                f"Tổng thời lượng các điểm ghim ({total_locked_duration} phút) vượt quá quỹ thời gian chuyến đi ({total_time_budget} phút). Hãy bỏ ghim bớt."
            )

        # 2. Fetch real distances from OSRM
        ...
        best_effort_haversine = False
        if getattr(self.distance_cache, "osrm_failed", False):
            if getattr(self.distance_cache, "cache_hit_rate", 0.0) == 0.0:
                raise SolverException(
                    ErrorCode.OSRM_UNREACHABLE,
                    "Máy chủ định tuyến OSRM đang ngoại tuyến và không có dữ liệu cache ngoại tuyến. Vui lòng thử lại sau."
                )
            else:
                best_effort_haversine = True

        if getattr(self.distance_cache, "had_fallback", False):
            best_effort_haversine = True

        # 3. Resolve hotel transfers
        self._resolve_hotel_transfers(day_plans, request.hotels)

        # 4. Solve trip (multi-depot, all days at once)
        ...
        if not days_result:
            raise SolverException(
                ...
                "Không thể tạo được lịch trình phù hợp với các ràng buộc hiện tại."
            )

        # 5. Budget validation with retry
        ...
        if best_effort_haversine:
            validation_notes.append("[WARNING] BEST_EFFORT_HAVERSINE: Một số chặng sử dụng ước tính đường chim bay do lỗi OSRM.")

        # 7. Post-solve rest break insertion
        ...
        return TravelItinerary(
            ...
            distance_type="haversine" if best_effort_haversine else "osrm",
        )
    ...
    def _validate_budget(
        ...
        for attempt in range(MAX_BUDGET_RETRIES):
            ...
            if not droppable:
                logger.warning("No droppable POIs left, budget validation failed.")
                raise SolverException(
                    ErrorCode.BUDGET_EXCEEDED,
                    f"Ngân sách {budget:,.0f} VND không đủ chi trả cho các địa điểm bắt buộc (ghim/ăn uống)."
                )
        ...
        if total_fee > budget:
            raise SolverException(
                ErrorCode.BUDGET_EXCEEDED,
                f"Chi phí vé vào cổng ({total_fee:,.0f} VND) vượt quá ngân sách tối đa ({budget:,.0f} VND)."
            )
