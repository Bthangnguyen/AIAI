    ...
    <div className="mx-auto w-full max-w-6xl space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {variants.map((variant) => {
          ...
          const isApplied = (selectedPlanStyle ?? "balanced") === variant.style
          const isLowestFatigue = variant.metrics.fatigueScore === minFatigue

          return (
            <div
              key={variant.style}
              className={`flex flex-col rounded-2xl border border-2 p-5 ${meta.border} ${meta.bg} ${isChillHighlight ? "ring-2 ring-emerald-300/70" : ""} ${isApplied ? "ring-2 ring-orange-400" : ""}`}
            >
